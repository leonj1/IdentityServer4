...
namespace build
{
    public class ProgramRunner
    {
        ...
        public static void RunTargetsAndExit(string[] args)
        {
            ...
            Target(Targets.CopyPackOutput, DependsOn(Targets.Pack), () =>
            {
                ...
                foreach (var file in Directory.GetFiles(packOutput, "*", SearchOption.AllDirectories))
                {
                    var relativePath = Path.GetRelativePath(packOutput, file);
                    var destinationFile = Path.Combine(packOutputCopy, relativePath);
